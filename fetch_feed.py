...
def categorize_entry(entry: dict) -> list[str]:
    ...
    if any(word in text for word in ["ai", "artificial intelligence", "machine learning", "automation"]):
        categories.append("AI")
    if any(word in text for word in ["b2b", "enterprise", "business-to-business"]):
        categories.append("B2B")
    if any(word in text for word in ["study", "research", "report", "survey", "academic", "institute"]):
